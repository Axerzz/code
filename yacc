...
一、读入处理
  读入minic.y
  Yacc结构：说明部分 %% 语法规则部分 %% 程序段部分
  第一部分%{和}%之间不做特殊处理
  处理%token，%left
  处理语法规则部分，首先是读取该产生式的左部，其次是冒号，然后一个右部，竖号，然后另一个右部，其实是另一个产生式，最后是分号，表示结束
  利用Lr1Item temp存储，并加入到vector<Lr1Item> Item中。
...
/*
 ...
 pdaState：自动机的项集
 pdaEdge：自动机的边
 1、初始化第一个项集init为S' -> program, #
 2、创建一个状态队列,存放尚未扩展等价项的状态queue<pdaState> stateQueue，并放入init
 3、创建一个已扩展等价项的状态集合，表示该集合中的状态都已扩展完成 set<pdaState> extendedState;
   set集合可以去重
 4、扩展每个项集中的等价项，while队列stateQueue不为空时
 {
    取出当前stateQueue中的top，为temp
      同理创建一个LR1项队列，存放尚未扩展等价项的LR1项queue<Lr1Item> itemQueue;
      创建一个已扩展等价项的LR1项集合，表示该集合中的LR1项均已扩展set<Lr1Item> extendedItem;
 5、遍历temp中的LR1项，如果需要扩展则加入itemQueue（即圆点后为非终结符）
 6、while队列itemQueue不为空时，对该LR1项扩展
      取出当前itemQueue中的top，为tempItem
      判断该LR1项是否可以扩展（圆点后是否为非终结符）
      求first集来确定预测符（即S->.ABα，sybmol=B，求first(B)或者first(Bα)）
      在存放LR1项的Item集合中遍历查找左部为当前LR1项非终结符的LR1项，将该LR1项加入temp
      ？？？判断该LR1项是否要加入itemQueue？？？
 7、求temp中各LR1项圆点后的符号，构成集合tag
 8、遍历tag中的符号，判断是否在temp中有LR1项的圆点后的终结符为该符号
   创建新state为tempstate，并将该LR1项圆点移动后加入到tempstate中。（参数赋值）
   创建边
   判断该tempstate是否为新状态，如果是直接将state和edge的参数赋值，并加入集合中
   如果不是，在extendState中找到对应的状态后，在进行操作。
 }
 */
...
{
    ofstream output("../cmake-build-debug/table.txt");
